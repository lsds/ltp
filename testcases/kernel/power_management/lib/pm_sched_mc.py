...
def set_timer_migration_interface(value):
    ...
    try:
        os.system(
            "echo %s > \
            /proc/sys/kernel/timer_migration 2>/dev/null"
            % value
        )
    except OSError as e:
...
def trigger_ebizzy(sched_smt, stress, duration, background, pinned):
    ...
    try:
        ...
        try:
            if background == "yes":
                succ = os.system(
                    "./ebizzy -t%s -s4096 -S %s >/dev/null &" % (threads, duration)
                )
            else:
                if pinned == "yes":
                    succ = os.system(
                        "taskset -c %s ./ebizzy -t%s -s4096 -S %s >/dev/null"
...
def trigger_kernbench(sched_smt, stress, background, pinned, perf_test):
    ...
    try:
        ...
        if linux_source_dir != "":
            ...
            print(
                "INFO: Linux kernel source not found in /root. Workload\
        ...
        if pinned == "yes":
            os.system(
                "taskset -c %s %s/kernbench -o %s -M -H -n 1 \
                >/dev/null 2>&1 &"
                % (cpu_count - 1, benchmark_path, threads)
            )
            ...
            if background == "yes":
                os.system(
                    "%s/kernbench -o %s -M -H -n 1 >/dev/null 2>&1 &"
...
def trigger_workld(
    sched_smt, workload, stress, duration, background, pinned, perf_test
):
    ...
    try:
        ...
        for pkg in sorted(cpu_map.keys()):
            ...
            print(
                "Package: ",
                pkg,
                "Idle %3.4f%%" % (float(total_idle) * 100 / total),
                file=reportfile,
            )
            print(
                "package-%s=%3.4f" % (pkg, (float(total_idle) * 100 / total)),
                file=keyvalfile,
            )
    except Exception as details:
        ...
        if is_hyper_threaded():
            ...
            for pkg in sorted(cpu_map.keys()):
                ...
                if len(cpu_group) == len(pkg_cpus):
                    ...
                    if (
                        int(cpus_utilized[0]) in cpu_map[pkg]
                        or int(cpus_utilized[1]) in cpu_map[pkg]
                    ):
                        return 0
            ...
            for i in range(0, len(lines)):
                if lines[i].endswith("CPU"):
                    ...
                    for j in range(0, len(subgroup)):
                        ...
                        status = validate_cpugrp_map(
                            cpu_group, sched_mc_level, sched_smt_level
                        )
                        if status == 1:
                            if is_quad_core() == 1:
                ...
                if work_ld == "kernbench" and sched_smt_level < sched_mc_level:
                    ...
                    if siblings != "":
                        ...
                        for i in range(0, len(sib_list)):
                            utilization += int(
                                get_cpu_utilization("cpu%s" % sib_list[i])
                            )
                    else:
