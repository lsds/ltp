...
def main(argv=None):
    ...
    parser.add_option(
        ...
        dest="stress",
        default="partial",
        help="Load on system is full/partial [i.e 50%]/thread",
    )
    parser.add_option(
        "-p",
        "--performance",
    ...
    try:
        ...
        if options.vary_mc_smt:
            ...
            if int(options.mc_value) < 2 and int(options.smt_value) < 2:
                trigger_ebizzy(
                    options.smt_value, "partial", duration, background, pinned
                )
                work_ld = "ebizzy"
                ...
                # When sched_mc & sched_smt is set
                trigger_kernbench(
                    options.smt_value, "partial", background, pinned, "no"
                )
                work_ld = "kernbench"
                import time
            ...
            generate_report()
            status = validate_cpu_consolidation(
                "partial", work_ld, options.mc_value, options.smt_value
            )
            if status == 0:
                ...
                if (
                    options.vary_mc_smt
                    and options.smt_value > 0
                    and is_hyper_threaded()
                ):
                ...
                generate_report()
                status = validate_cpu_consolidation(
                    "partial", work_ld, mc_value, smt_value
                )
                if background == "yes":
                    stop_wkld(work_ld)
                ...
                print(
                    "INFO: CPU consolidation failed when sched_mc &(/) \
sched_smt was enabled. This is pre-requisite to proceed"
            ...
            generate_report()
            status = validate_cpu_consolidation(
                options.stress, options.work_ld, options.mc_value, options.smt_value
            )
            reset_schedmc()
            if is_hyper_threaded():
                reset_schedsmt()
