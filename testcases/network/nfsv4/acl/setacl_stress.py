...
parser.add_option(
    "-n", "--nloop", dest="nloop", type="int", help="number of loop to do in the test"
)
parser.add_option(
    "-p", "--path", dest="path", help="path on which the test is executed"
)
parser.add_option(
    "-f",
    "--nbfiles",
    dest="nbfiles",
    type="int",
    help="nb of files to do the test (default=5)",
)
parser.add_option(
    "-u", "--nbusers", dest="nbusers", type="int", help="nb of users (default=5)"
)
parser.add_option(
    "-g", "--nbgrp", dest="nbgroups", type="int", help="nb of groups (default=5)"
)
