...
class Client(Machine):
    ...
    def isomount(self, dir):
        ...
        mntpoint = NFS4_PATH
        self.command = (
            "'mkdir -p " + mntpoint + "; mount -t nfs4 " + export + " " + mntpoint + "'"
        )
        self.do()
...
def usage():
    ...
    print("usage:")
    print(
        "locktests.py <-n process -f testfile ><--setup -s fs_server> -c host1, host2, host3 ... "
    )
    print(
    ...
    print(
        "-n <num>         : number of processes each test machine will lauch to perform the test"
    )
    ...
    print(
        "./locktests.py --setup -c testmachine1 testmachine2 testmachine3 -s my_nfs_server:/"
    )
    print("\n")
    print(
        "*Run test on testmachine1,testmachine2 with 50 process on each machine using /mnt/nfsv4/testfile"
    )
